<template>
  <div class="app-container">
    <div class="filter-container" style="padding-bottom:20px;">
      <el-input v-model="tapInfo.sn" placeholder="名称 SN" style="width: 400px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-button v-waves class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">
        搜索
      </el-button>
      <el-button v-waves class="filter-item" type="primary" icon="el-icon-plus" @click="handleClockAdd">
        添加
      </el-button>
    </div>

    <el-table
      v-loading="clockListLoading"
      :data="clockList">
      ...
      <el-table-column prop="open_time" label="开启时间" align="center"> </el-table-column>
      <el-table-column prop="close_time" label="关闭时间" align="center"> </el-table-column>
      <el-table-column label="状态" align="center">
        <template slot-scope="scope">
          <el-tag type="success" v-if="scope.row.alive==1">激活</el-tag>
          <el-tag type="danger" v-if="scope.row.alive==0">未激活</el-tag>
        </template>
      </el-table-column>
      ...
      <el-form :model="clockForm">
        <el-form-item label="重复" :label-width="formLabelWidth">
          <el-select v-model="value" placeholder="请选重复周期">
            <el-option label="执行一次" value="1"></el-option>
            <el-option label="每天" value="2"></el-option>
            <el-option label="工作日" value="3"></el-option>
            <el-option label="周末" value="4"></el-option>
            <el-option label="自定义" value="5"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="开启时间" :label-width="formLabelWidth">
          <el-time-picker
            v-model="clockForm.open_time"
            value-format="HH:mm"
            placeholder="任意时间点">
          </el-time-picker>
        </el-form-item>
...
<script>
import { tapClockList, tapClockDelete, tapClockEdit, tapClockAdd } from '@/api/tap'
...
export default {
  ...
  methods: {
    ...
    handleClockAddOk() {
      ...
      if (this.clockForm.open_time.length > 1 || this.clockForm.close_time.length > 1) {
